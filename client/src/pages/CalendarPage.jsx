...
function BookingBlock({ b, roomColor, compact }) {
  ...
  return (
    <div style={{
      ...
      {dur >= 45 && (
        <div style={{ fontSize: '0.68rem', color: 'var(--tx)', fontWeight: 500, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', marginTop: 1 }}>
          {b.purpose}
        </div>
      )}
      {dur >= 75 && (
        <div style={{ fontSize: '0.63rem', color: 'var(--tx-3)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
          {b.name}
        </div>
      )}
    </div>
...
function WeekView({ weekDays, bookings, rooms, navigate }) {
  ...
  return (
    <div style={{ overflowX: 'auto' }}>
      {/* Day headers */}
      <div style={{ display: 'grid', gridTemplateColumns: '52px repeat(7, 1fr)', borderBottom: '1px solid var(--line)', background: 'var(--bg-2)', position: 'sticky', top: 0, zIndex: 10 }}>
        <div />
        {weekDays.map((d, i) => {
          const today = isToday(d)
          return (
            <div key={i} style={{ textAlign: 'center', padding: '10px 4px', borderLeft: '1px solid var(--line)' }}>
              <div style={{ fontSize: '0.7rem', fontWeight: 600, textTransform: 'uppercase', color: today ? 'var(--accent)' : 'var(--tx-3)', letterSpacing: '0.06em' }}>
                {DAY_NAMES[i]}
              </div>
              <div style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '52px repeat(7, 1fr)', minWidth: 600 }}>
        ...
        <div style={{ position: 'relative', height: HOURS.length * HOUR_H }}>
          {HOURS.map((h, i) => (
            <div key={h} style={{ position: 'absolute', top: i * HOUR_H - 8, right: 8, fontSize: '0.62rem', color: 'var(--tx-3)', fontWeight: 500 }}>
              {fmtHour(h)}
            </div>
          ))}
        </div>
        ...
        {weekDays.map((d, di) => {
          ...
          return (
            ...
            >
              {/* Hour lines */}
              {HOURS.map((_, i) => (
                <div key={i} style={{ position: 'absolute', top: i * HOUR_H, left: 0, right: 0, height: 1, background: 'var(--line)', opacity: 0.6 }} />
              ))}
              {/* Half-hour lines */}
              {HOURS.map((_, i) => (
                <div key={`h${i}`} style={{ position: 'absolute', top: i * HOUR_H + HOUR_H / 2, left: 0, right: 0, height: 1, background: 'var(--line)', opacity: 0.25 }} />
              ))}
    ...
    <div style={{ display: 'grid', gridTemplateColumns: selectedDay ? '1fr 300px' : '1fr', gap: 16, alignItems: 'start' }}>
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', marginBottom: 4 }}>
          {DAY_NAMES.map(d => (
            <div key={d} style={{ textAlign: 'center', padding: '6px 0', fontSize: '0.7rem', fontWeight: 700, color: 'var(--tx-3)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
              {d}
            </div>
          ))}
        </div>
        ...
        {/* Month grid */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', border: '1px solid var(--line)', borderRadius: 12, overflow: 'hidden' }}>
          {monthGrid.map((d, i) => {
            const dateStr = toDateStr(d)
            ...
            return (
              <div
                ...
                style={{
                  ...
                  background: selected ? 'var(--accent-subtle)' : today ? 'rgba(124,106,247,0.04)' : 'var(--bg-2)',
                  cursor: 'pointer',
              ...
              >
                <div style={{
                  ...
                  background: today ? 'var(--accent)' : selected ? 'var(--accent-text)' : 'transparent',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontSize: '0.8rem', fontWeight: today || selected ? 700 : 400,
                  color: today || selected ? 'white' : inMonth ? 'var(--tx)' : 'var(--tx-3)',
                  marginBottom: 4,
                }}>
                  {d.getDate()}
                </div>
                ...
                <div style={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
                  ...
                  {dayBookings.length > 3 && (
                    <div style={{ fontSize: '0.6rem', color: 'var(--tx-3)', padding: '1px 4px' }}>
        ...
        <div className="card" style={{ position: 'sticky', top: 24 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
            <div>
              ...
              <div style={{ fontSize: '0.78rem', color: 'var(--tx-3)' }}>
                {selectedDay.toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
              </div>
            </div>
            <button style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--tx-3)', fontSize: '1.2rem', lineHeight: 1 }}
              ...
              {selectedBookings.sort((a, b) => a.startTime.localeCompare(b.startTime)).map(b => (
                <div key={b.id} style={{ padding: '10px 0', borderBottom: '1px solid var(--line)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 2 }}>
                    ...
                    <span style={{ fontSize: '0.82rem', fontWeight: 700, color: 'var(--tx)' }}>{toAMPM(b.startTime)} – {toAMPM(b.endTime)}</span>
                  </div>
                  <div style={{ fontSize: '0.82rem', color: 'var(--tx)', paddingLeft: 16 }}>{b.purpose}</div>
                  <div style={{ fontSize: '0.72rem', color: 'var(--tx-3)', paddingLeft: 16, marginTop: 2 }}>{b.name} · {b.room?.name}</div>
                </div>
              ))}
    ...
    <div className="page animate-in" style={{ maxWidth: '100%' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20, gap: 12, flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <button onClick={prev} style={{ background: 'var(--bg-3)', border: '1px solid var(--line)', borderRadius: 8, width: 34, height: 34, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--tx-2)', fontSize: '1rem' }}>‹</button>
          <span style={{ fontFamily: 'var(--font-display)', fontSize: '1.25rem', fontWeight: 600, minWidth: 200, textAlign: 'center' }}>{label}</span>
          <button onClick={next} style={{ background: 'var(--bg-3)', border: '1px solid var(--line)', borderRadius: 8, width: 34, height: 34, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--tx-2)', fontSize: '1rem' }}>›</button>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            {rooms.map(r => (
              <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                ...
                <span style={{ fontSize: '0.72rem', color: 'var(--tx-3)' }}>{r.name}</span>
              </div>
            ))}
          </div>

          {/* View toggle */}
          <div style={{ display: 'flex', background: 'var(--bg-3)', border: '1px solid var(--line)', borderRadius: 8, padding: 3, gap: 2 }}>
            {['week', 'month'].map(v => (
              <button key={v} onClick={() => setView(v)} style={{
                padding: '5px 14px', borderRadius: 6, border: 'none', cursor: 'pointer',
                background: view === v ? 'var(--bg-2)' : 'transparent',
                color: view === v ? 'var(--tx)' : 'var(--tx-3)',
                fontFamily: 'var(--font)', fontSize: '0.8rem', fontWeight: view === v ? 600 : 400,
                boxShadow: view === v ? 'var(--s2)' : 'none', transition: 'all 0.15s',
              }}>
